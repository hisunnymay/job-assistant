...
def test_service_passes_static_pdf_to_ai_boundary_without_extraction(
    db_session: Session,
) -> None:
    resume_path = (
    ...
    result = service.generate("用于验证静态 PDF 交接与持久化的完整职位描述。" * 3)

    assert ai_service.resume_path == resume_path
    assert ai_service.resume_path.is_file()
